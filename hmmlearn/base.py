...
class _BaseHMM(BaseEstimator):
    ...
    def online_fit(self, X, lengths):
        ...
        self : object
            Returns self.
        """
        # both not reliant on X or lengths, only do once
        self._online_init(X, lengths=lengths)
        self._check()
        ...
        for iter in range(self.n_iter):
            ...
            for i, j in iter_from_X_lengths(X, lengths):
                ...
                posteriors = self._compute_posteriors(fwdlattice, bwdlattice)
                self._accumulate_sufficient_statistics(
                    stats, sequence, framelogprob, posteriors, fwdlattice,
                    bwdlattice)

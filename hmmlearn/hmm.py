...
class MultinomialHMM(_BaseHMM):
    ...
    def _online_init(self, X, lengths=None):
        if not self._online_check_input_symbols(X):
            raise ValueError("expected a sample from "
                             "a Multinomial distribution.")
    ...
    def _online_check_input_symbols(self, X):
        """Check if ``X`` is a sample from a Multinomial distribution.
        ...
        Multinomial distribution, while ``[0, 0, 3, 5, 10]`` is not.
        """
        symbols = [symbol[0] for symbol in X]
        for symbol in symbols:
            if isinstance(symbol, int) or np.issubdtype(symbol, np.integer):
                if symbol < 0:
                    print(symbol, isinstance(symbol, int),
                          np.issubdtype(symbol, np.integer))
                    return False
            else:
                print(symbol, isinstance(symbol, int),
                      np.issubdtype(symbol, np.integer))
